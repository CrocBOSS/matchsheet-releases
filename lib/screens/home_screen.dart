...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<bool> _handleBackButton() async {
    if (hasUnsavedChanges) {
          ...
          return AlertDialog(
            ...
            actions: [
              ...
              TextButton(
                ...
                child: const Text('Save & Exit', style: TextStyle(fontSize: 12, color: Colors.blue)),
              ),
              TextButton(
                onPressed: () {
                  Navigator.pop(dialogContext);
      ...
      // If user chose Save & Exit, show the save dialog
      if (saveAndExit && mounted) {
        await _showSaveGameDialogForExit();
      }
      
      return false; // Dialog handling is complete in the dialog itself
    } else {
  ...
  Future<void> _showSaveGameDialogForExit() async {
    final saveNameController = TextEditingController();
      ...
      builder: (dialogContext) {
        return StatefulBuilder(
          builder: (stateContext, setState) {
            return AlertDialog(
              title: const Text('Save Game Before Exit', style: TextStyle(fontSize: 16)),
              contentPadding: const EdgeInsets.fromLTRB(16, 12, 16, 12),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  const Text(
                    'Save this match before exiting?',
                    style: TextStyle(fontSize: 12),
                  ),
                  const SizedBox(height: 12),
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      border: const OutlineInputBorder(),
                      contentPadding: const EdgeInsets.symmetric(horizontal: 8, vertical: 8),
                      hintText: currentSessionName != null 
                        ? 'Leave blank to update "$currentSessionName"'
                        : 'Enter name or leave blank to skip',
                      hintStyle: TextStyle(fontSize: 11, color: Colors.grey[500]),
                    ),
                    style: const TextStyle(fontSize: 12),
                  ),
              ...
              actions: [
                ...
                TextButton(
                  onPressed: () async {
                    String gameName = saveNameController.text;
                    bool shouldSave = false;

                    // Determine if we should save
                    if (gameName.isNotEmpty) {
                      shouldSave = true;
                    } else if (currentSessionName != null && players.isNotEmpty) {
                      gameName = currentSessionName!;
                      shouldSave = true;
                    }
                    ...
                    if (shouldSave) {
                      try {
                        // First delete the old session if updating
                        if (currentSessionName != null && gameName == currentSessionName) {
                          final sessions = await StorageService.loadSavedSessions();
                          final index = sessions.indexWhere((s) => s['name'] == currentSessionName);
                          if (index >= 0) {
                            await StorageService.deleteSession(index);
                  ...
                  child: Text(
                    saveNameController.text.isNotEmpty
                        ? 'Save & Exit'
                        : (currentSessionName != null && players.isNotEmpty)
                            ? 'Update & Exit'
                            : 'Exit Without Saving',
    ...
    return PopScope(
      ...
      appBar: AppBar(
        ...
        title: screenWidth > 600
            ...
            : Tooltip(
                message: 'Pep Match Statistics Tracker',
                child: Container(
                  decoration: BoxDecoration(
                    border: Border.all(color: Colors.white),
                    borderRadius: BorderRadius.circular(4),
